...
// ============================================================
// Teacher AI API — Gemini FREE tier, multi-key rotation
// รองรับ GEMINI_API_KEY, GEMINI_API_KEY_2 ... GEMINI_API_KEY_5
// เมื่อ key ใดโดน 429/503 → หมุนไป key ถัดไปอัตโนมัติ
// ============================================================

// Gemini free-tier models — เรียงจาก stable → fallback
// ตรวจสอบจาก ListModels API เมื่อ 2025-04 ว่าใช้ generateContent ได้
const GEMINI_MODELS = [
  'gemini-2.0-flash',          // primary — 1,500 RPD, 15 RPM
  'gemini-2.0-flash-lite',     // fallback — 1,500 RPD, 30 RPM (เร็วกว่า)
  'gemini-2.5-flash-lite',     // fallback — รุ่นใหม่ lite
  'gemini-2.5-flash',          // fallback — ฉลาดกว่า แต่ 250 RPD
  'gemini-flash-latest',       // alias → ชี้ไป flash รุ่นใหม่สุดเสมอ
  'gemini-flash-lite-latest',  // alias → ชี้ไป flash-lite รุ่นใหม่สุดเสมอ
];
...
// รวม keys ทุกตัวที่มี (ใส่เพิ่มใน Vercel ได้เลย)
function getApiKeys() {
  const keys = [];
  for (let i = 1; i <= 10; i++) {
    const k = i === 1
...
async function callGemini(prompt, imageData = null) {
  ...
  if (keys.length === 0) throw new Error('ไม่มี GEMINI_API_KEY — กรุณาตั้งค่าใน Vercel Environment Variables');
  ...
  // วน key × model — key หมดก่อน แล้วค่อยเปลี่ยน model
  for (const model of GEMINI_MODELS) {
    for (const apiKey of keys) {
      try {
        const url = `https://generativelanguage.googleapis.com/v1beta/models/${model}:generateContent?key=${apiKey}`;
        console.log(`[AI] ${model} key[${keys.indexOf(apiKey) + 1}/${keys.length}]`);
        ...
        if (res.status === 429 || res.status === 503) {
          console.warn(`[AI] ${model} key${keys.indexOf(apiKey) + 1} rate limited → next`);
        ...
        if (text) {
          console.log(`[AI] ✅ ${model} key${keys.indexOf(apiKey) + 1}`);
          return text;
        }
      } catch (e) {
  ...
  // ทุก key + ทุก model ล้มเหลว
  throw lastError?.message === 'rate_limited'
    ? new Error('🔴 AI ใช้งานเกินโควต้าวันนี้แล้ว กรุณาลองใหม่พรุ่งนี้ หรือติดต่อผู้ดูแลระบบ')
    : (lastError || new Error('AI ไม่สามารถใช้งานได้ กรุณาลองใหม่'));
}
...
- **ห้าม**ขึ้นต้นคำถามด้วยวลีอ้างถึงเวลา/ข้อมูล เช่น "จากข้อมูล ณ เวลา X วิดีโอกล่าวถึง", "ตามที่วิดีโอ", "จากคลิป", "ในนาทีที่ X"
- ให้เริ่มต้นด้วยคำถามตรงๆ เลย เช่น "หลักการบริหารโครงการประการใดสำคัญที่สุด?", "การวางแผนโครงการมีองค์ประกอบใด?", "Project Manager มีบทบาทอย่างไร?"`;
        break;
      }
