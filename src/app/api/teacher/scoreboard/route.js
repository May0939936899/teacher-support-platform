...
// Safe timestamp → ms converter (handles ISO string, number, or Date)
function toMs(val) {
...
export async function GET(request) {
  try {
    ...
    if (expiresAt !== null && Date.now() > expiresAt) return NextResponse.json({ error: 'Session หมดเวลาแล้ว' }, { status: 400 });
