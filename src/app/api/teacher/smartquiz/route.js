...
// Convert any timestamp (ISO string, number ms, or Date) → number ms
// Returns null if value is falsy or invalid
function toMs(val) {
...
export async function GET(request) {
  try {
    ...
    if (!session.active || (expiresAt !== null && now > expiresAt)) {
      return NextResponse.json({ error: 'Session หมดเวลาแล้ว หรือถูกปิดโดยอาจารย์' }, { status: 400 });
    }
    if (opensAt !== null && now < opensAt) {
      const opensDate = new Date(opensAt).toLocaleString('th-TH', { dateStyle: 'short', timeStyle: 'short' });
...
export async function POST(request) {
  try {
    ...
    const now = Date.now();
    // Store as ISO strings so Supabase TIMESTAMPTZ accepts them correctly
    const opensAtMs = openAt ? toMs(openAt) : now;
    const expiresAtMs = closeAt ? toMs(closeAt) : (now + (timeLimit || 30) * 60000);
    const session = {
      id: code,
      quiz,
      responses: [],
...
export async function PATCH(request) {
  try {
    ...
    const patchExpiresAt = toMs(session.expires_at);
    const patchOpensAt = toMs(session.opens_at);
    if (!session.active || (patchExpiresAt !== null && nowPatch > patchExpiresAt)) {
      return NextResponse.json({ error: 'Session หมดเวลาแล้ว' }, { status: 400 });
    }
    if (patchOpensAt !== null && nowPatch < patchOpensAt) {
  ...
  try {
    ...
    if (action === 'closed') {
      ...
      for (const s of sessions) {
        if (!s.active || (toMs(s.expires_at) !== null && Date.now() >= toMs(s.expires_at))) {
