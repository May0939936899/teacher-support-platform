...
export default function VideoQuiz() {
  ...
  // Check URL params for auto-join — lock to student mode
  useEffect(() => {
  ...
  // ===== MODE SELECTION (only show when NOT locked) =====
  ...
  return (
    <div style={{ padding: '24px', maxWidth: '1100px', margin: '0 auto', fontFamily: FONT }}>
      {/* Back button — hidden when student locked */}
      {!isStudentLocked && (
      ...
      {/* ===== TEACHER MODE (hidden when student locked) ===== */}
      {mode === 'teacher' && !isStudentLocked && !publishedCode && (
      ...
      {/* ===== TEACHER: Published View (hidden when student locked) ===== */}
      {mode === 'teacher' && !isStudentLocked && publishedCode && (
      ...
      {mode === 'student' && (
        <div style={{ maxWidth: '800px', margin: '0 auto' }}>
          {/* Join screen — require name + student ID */}
          {!studentQuiz && (
            <div style={{ background: '#fff', borderRadius: '24px', padding: '44px', border: '1px solid #e2e8f0', textAlign: 'center', boxShadow: '0 4px 20px rgba(0,0,0,0.06)' }}>
              ...
              <p style={{ color: '#94a3b8', fontSize: '15px', marginBottom: '28px' }}>กรอกข้อมูลเพื่อเข้าทำ Quiz</p>
              ...
              <div style={{ maxWidth: '400px', margin: '0 auto', display: 'flex', flexDirection: 'column', gap: '16px' }}>
                ...
                <div>
                  <label style={{ ...lbl, textAlign: 'left' }}>รหัสนักศึกษา *</label>
                  <input placeholder="เช่น 6512345678" value={studentId}
                    onChange={e => setStudentId(e.target.value)} style={inp}
                    onFocus={e => e.target.style.borderColor = CI.purple} onBlur={e => e.target.style.borderColor = '#e2e8f0'} />
                </div>
                <div>
                  <label style={{ ...lbl, textAlign: 'left' }}>ชื่อ-นามสกุล *</label>
                ...
                <button onClick={joinQuiz} disabled={!studentCode || !studentName || !studentId} style={{
                  padding: '16px', borderRadius: '14px', border: 'none',
                  background: studentCode && studentName && studentId ? `linear-gradient(135deg, ${CI.cyan}, ${CI.magenta})` : '#e2e8f0',
                  color: studentCode && studentName && studentId ? '#fff' : '#94a3b8',
                  cursor: studentCode && studentName && studentId ? 'pointer' : 'not-allowed',
          ...
          {/* Quiz in progress — wrapped with AntiCheatGuard */}
          {studentQuiz && !studentSubmitted && (
            <AntiCheatGuard active={true} maxWarnings={1} onViolation={() => { submitQuiz(); }}>
              <div>
                {/* Video */}
                {studentQuiz.ytId && (
                ...
                {/* Student info badge */}
                <div style={{
                  background: '#f0f9ff', borderRadius: '12px', padding: '12px 18px', marginBottom: '12px',
                  border: '1px solid #bae6fd', display: 'flex', alignItems: 'center', gap: '16px',
                  fontSize: '14px', color: '#0369a1',
                }}>
                  <span>🎓 <strong>{studentName}</strong></span>
                  <span style={{ color: '#64748b' }}>|</span>
                  <span>🎫 รหัส: <strong>{studentId}</strong></span>
                </div>
