...
// ============================================================
// EXPORT TO EXCEL — ไม่ต้องใช้ library ใดๆ
// ใช้ HTML table + application/vnd.ms-excel (Excel เปิดได้ รองรับภาษาไทย)
// ============================================================
function buildExcelFile(session) {
  const quiz = session.quiz;
  const responses = [...(session.responses || [])].sort((a, b) => b.score - a.score);
  const total = (quiz.questions || []).reduce((a, q) => a + (q.points || 1), 0);
  const passScore = Math.ceil(total * 0.5);

  const esc = (str) => String(str ?? '').replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;');

  const headerRow = ['#', 'รหัสนักศึกษา', 'ชื่อ-นามสกุล', 'คะแนน', 'คะแนนเต็ม', 'เปอร์เซ็นต์', 'สถานะ', 'วันที่-เวลาส่ง']
    .map(h => `<th style="background:#004175;color:#fff;font-weight:bold;padding:8px 12px;border:1px solid #ccc">${esc(h)}</th>`).join('');

  const bodyRows = responses.map((r, i) => {
    const pct = total > 0 ? Math.round((r.score / total) * 100) : 0;
    const pass = r.score >= passScore;
    const bg = i % 2 === 0 ? '#ffffff' : '#f0f9ff';
    const cells = [
      i + 1, r.studentId || '-', r.studentName || 'ไม่ระบุ',
      r.score, total, `${pct}%`,
      pass ? 'ผ่าน' : 'ไม่ผ่าน',
      new Date(r.submittedAt).toLocaleString('th-TH'),
    ].map(v => `<td style="padding:7px 12px;border:1px solid #e2e8f0;background:${bg}">${esc(v)}</td>`).join('');
    return `<tr>${cells}</tr>`;
  }).join('');
...
<x:Name>ผลคะแนน</x:Name><x:WorksheetOptions><x:DisplayGridlines/></x:WorksheetOptions>
</x:ExcelWorksheet></x:ExcelWorksheets></x:ExcelWorkbook></xml><![endif]-->
<style>table{border-collapse:collapse;font-family:Tahoma,sans-serif;font-size:13px}th,td{white-space:nowrap}</style>
</head><body>
<h2 style="color:#004175;margin-bottom:4px">${esc(quiz.title)}</h2>
<p style="color:#64748b;margin-bottom:16px">Session: ${esc(session.id)} · ผู้สอบ ${responses.length} คน · คะแนนเต็ม ${total}</p>
<table><thead><tr>${headerRow}</tr></thead><tbody>${bodyRows}</tbody></table>
  ...
  a.href = url;
  a.download = `คะแนน_${session.id}_${(quiz.title || 'quiz').replace(/[^\u0E00-\u0E7Fa-zA-Z0-9]/g, '_')}.xls`;
  a.click();
  setTimeout(() => URL.revokeObjectURL(url), 1000);
}

// ============================================================
// RESULTS MODAL — ดูคะแนนเต็มรูปแบบ + Export Excel
// ============================================================
function ResultsModal({ session, onClose }) {
  const CI_local = { cyan: '#00b4e6', purple: '#7c4dff' };
  const quiz = session.quiz;
  const responses = [...(session.responses || [])].sort((a, b) => b.score - a.score);
  const total = (quiz.questions || []).reduce((a, q) => a + (q.points || 1), 0);
  const avg = responses.length > 0 ? responses.reduce((a, r) => a + r.score, 0) / responses.length : 0;
  const passCount = responses.filter(r => r.score >= Math.ceil(total * 0.5)).length;

  return (
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.65)', zIndex: 9999, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 16, fontFamily: "'Kanit','Noto Sans Thai',sans-serif" }}
      onClick={onClose}>
      <div style={{ background: '#fff', borderRadius: 20, width: '100%', maxWidth: 720, maxHeight: '92vh', display: 'flex', flexDirection: 'column', boxShadow: '0 24px 64px rgba(0,0,0,0.3)', overflow: 'hidden' }}
        ...
        <div style={{ padding: '20px 24px', borderBottom: '1px solid #e2e8f0', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexShrink: 0 }}>
          <div>
            <h2 style={{ margin: 0, fontSize: 19, fontWeight: 800, color: '#1e293b' }}>📊 {quiz.title}</h2>
            <p style={{ margin: '3px 0 0', color: '#94a3b8', fontSize: 13 }}>Code: <b>{session.id}</b> · {responses.length} คน ส่งแล้ว</p>
          </div>
          <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
            {responses.length > 0 && (
              <button onClick={() => buildExcelFile(session)} style={{
                padding: '9px 16px', borderRadius: 10, border: '1.5px solid #16a34a',
                background: '#f0fdf4', color: '#16a34a', cursor: 'pointer',
                fontSize: 14, fontWeight: 700, fontFamily: 'inherit', display: 'flex', alignItems: 'center', gap: 6,
              }}>📥 Excel</button>
            )}
            <button onClick={onClose} style={{
              width: 36, height: 36, borderRadius: 10, border: '1px solid #e2e8f0',
              background: '#f8fafc', cursor: 'pointer', fontSize: 18, color: '#64748b', fontFamily: 'inherit',
            }}>✕</button>
          </div>
        </div>
        ...
        {/* Stats bar */}
        {responses.length > 0 && (
          <div style={{ padding: '14px 24px', background: '#f8fafc', display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 10, borderBottom: '1px solid #e2e8f0', flexShrink: 0 }}>
            {[
              { l: 'ส่งแล้ว', v: responses.length, u: 'คน', c: CI_local.cyan },
              { l: 'คะแนนเฉลี่ย', v: avg.toFixed(1), u: `/${total}`, c: CI_local.purple },
              { l: 'ผ่าน (≥50%)', v: passCount, u: 'คน', c: '#16a34a' },
              { l: 'อัตราผ่าน', v: `${responses.length > 0 ? Math.round(passCount / responses.length * 100) : 0}%`, u: '', c: '#f59e0b' },
            ].map(s => (
              <div key={s.l} style={{ background: '#fff', borderRadius: 10, padding: '10px 12px', textAlign: 'center', border: '1px solid #e2e8f0' }}>
                <div style={{ fontSize: 20, fontWeight: 800, color: s.c }}>{s.v}<span style={{ fontSize: 12, fontWeight: 500, color: '#94a3b8' }}>{s.u}</span></div>
        ...
        <div style={{ flex: 1, overflow: 'auto', padding: '0 24px 24px' }}>
          {responses.length === 0 ? (
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 14, marginTop: 16 }}>
              <thead>
                <tr>
                  {['#', 'รหัสนักศึกษา', 'ชื่อ-นามสกุล', 'คะแนน', '%', 'สถานะ', 'เวลา'].map(h => (
                    <th key={h} style={{ padding: '10px 12px', textAlign: 'left', fontWeight: 700, color: '#64748b', borderBottom: '2px solid #e2e8f0', whiteSpace: 'nowrap', background: '#f8fafc' }}>{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                {responses.map((r, i) => {
                  const pct = total > 0 ? Math.round((r.score / total) * 100) : 0;
                  const pass = r.score >= Math.ceil(total * 0.5);
                  return (
                    <tr key={i} style={{ borderBottom: '1px solid #f1f5f9' }}>
                      <td style={{ padding: '10px 12px', color: '#94a3b8', fontWeight: 700 }}>{i + 1}</td>
                      <td style={{ padding: '10px 12px', fontWeight: 600, color: '#1e293b' }}>{r.studentId || '-'}</td>
                      <td style={{ padding: '10px 12px', color: '#374151' }}>{r.studentName || 'ไม่ระบุ'}</td>
                      <td style={{ padding: '10px 12px', fontWeight: 800, color: CI_local.cyan }}>{r.score}<span style={{ fontWeight: 400, color: '#94a3b8' }}>/{total}</span></td>
                      <td style={{ padding: '10px 12px' }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                          <div style={{ flex: 1, height: 6, borderRadius: 99, background: '#e2e8f0', overflow: 'hidden', minWidth: 40 }}>
                            <div style={{ height: '100%', width: `${pct}%`, background: pass ? '#10b981' : '#f59e0b', borderRadius: 99 }} />
                          </div>
                          <span style={{ fontSize: 12, color: '#64748b', flexShrink: 0 }}>{pct}%</span>
                        </div>
                      </td>
                      <td style={{ padding: '10px 12px' }}>
                        <span style={{ padding: '3px 9px', borderRadius: 6, fontSize: 12, fontWeight: 700, background: pass ? '#dcfce7' : '#fee2e2', color: pass ? '#16a34a' : '#ef4444' }}>
                          {pass ? '✓ ผ่าน' : '✗ ไม่ผ่าน'}
                        </span>
                      </td>
                      <td style={{ padding: '10px 12px', color: '#94a3b8', fontSize: 12, whiteSpace: 'nowrap' }}>
                        {new Date(r.submittedAt).toLocaleTimeString('th-TH', { hour: '2-digit', minute: '2-digit' })}
                      </td>
                    </tr>
                  );
                })}
...
export default function SmartQuiz() {
  ...
  // Auto-poll active session every 5s to update live results
  useEffect(() => {
    if (!activeSession?.active) return;
    const poll = async () => {
      try {
        const res = await fetch(`${QUIZ_API}?action=list`);
        const data = await res.json();
        if (data.sessions) {
          setSessions(data.sessions);
          const fresh = data.sessions.find(s => s.id === activeSession.id);
          if (fresh) setActiveSession(prev => ({ ...prev, ...fresh }));
        }
      } catch {}
    };
  ...
  return (
    <div style={{ padding: '24px', maxWidth: '1100px', margin: '0 auto', fontFamily: FONT }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: activeSession ? '1fr 380px' : '1fr', gap: '24px' }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(175px, 1fr))', gap: '12px', marginBottom: '20px' }}>
                ...
                >
                  <div style={{ display: 'flex', alignItems: 'center', gap: '10px', justifyContent: 'center' }}>
                    ...
                    <div style={{ textAlign: 'left' }}>
                      <div style={{ fontSize: '15px', fontWeight: 700, color: createMode === 'import' ? '#d97706' : '#1e293b' }}>วางข้อสอบ พร้อมเฉลย</div>
                      <div style={{ fontSize: '12px', color: '#94a3b8', lineHeight: 1.4 }}>จัดข้ออัตโนมัติ เปิด Session ได้ทันที</div>
                    </div>
            ...
            {createMode === 'import' && (
              <div style={{ background: '#fff', borderRadius: '16px', padding: '24px', border: '2px solid #fde68a', marginBottom: '20px' }}>
                <h4 style={{ margin: '0 0 6px', fontSize: '18px', color: '#1e293b', fontWeight: 700 }}>
                  📋 วางข้อสอบ พร้อมเฉลย — จัดข้ออัตโนมัติ
                </h4>
                <p style={{ color: '#64748b', fontSize: '13px', margin: '0 0 14px', lineHeight: 1.6 }}>
                  วางข้อสอบที่มีตัวเลือกและเฉลยครบ ระบบจะแยกข้อ ตัวเลือก และเฉลยให้เอง เปิด Session ได้ทันที
                </p>
          ...
          {activeSession && (
            <div>
              ...
              {/* Live results — แสดงชื่อ+รหัส ไม่แสดงคะแนน */}
              {(() => {
                const liveSession = sessions.find(s => s.id === activeSession.id) || activeSession;
                const liveResponses = [...(liveSession?.responses || [])].sort((a, b) => b.submittedAt - a.submittedAt);
                return (
                  <div style={{ background: '#fff', borderRadius: '16px', padding: '20px', border: '1px solid #e2e8f0' }}>
                    {/* Header */}
                    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '14px' }}>
                      <h3 style={{ margin: 0, fontSize: '15px', color: '#1e293b', fontWeight: 700, display: 'flex', alignItems: 'center', gap: 8 }}>
                        📋 ผลแบบ Live
                        <span style={{
                          background: liveResponses.length > 0 ? '#dcfce7' : '#f1f5f9',
                          color: liveResponses.length > 0 ? '#16a34a' : '#94a3b8',
                          borderRadius: 8, padding: '2px 10px', fontSize: 13, fontWeight: 700,
                        }}>
                          {liveResponses.length} คน
                        </span>
                      </h3>
                      <div style={{ display: 'flex', gap: 6 }}>
                        {liveResponses.length > 0 && (
                          <>
                            <button onClick={() => setViewingSession(liveSession)} style={{
                              padding: '5px 10px', borderRadius: 8, border: `1px solid ${CI.cyan}`, background: `${CI.cyan}10`,
                              color: CI.cyan, cursor: 'pointer', fontSize: 12, fontWeight: 700, fontFamily: 'inherit',
                            }}>🔍 ดูคะแนน</button>
                            <button onClick={() => buildExcelFile(liveSession)} style={{
                              padding: '5px 10px', borderRadius: 8, border: '1px solid #16a34a', background: '#f0fdf4',
                              color: '#16a34a', cursor: 'pointer', fontSize: 12, fontWeight: 700, fontFamily: 'inherit',
                            }}>📥 Excel</button>
                          </>
                        )}
                      </div>
                    ...
                    {/* Student list — ชื่อ + รหัส เท่านั้น */}
                    <div style={{ display: 'flex', flexDirection: 'column', gap: 7, maxHeight: 300, overflowY: 'auto' }}>
                        ...
                        <div key={i} style={{
                          ...
                          <div style={{ flex: 1, minWidth: 0 }}>
                            ...
                            <div style={{ fontSize: 11, color: '#94a3b8', marginTop: 1 }}>
                              {r.studentId || '-'} · {new Date(r.submittedAt).toLocaleTimeString('th-TH', { hour: '2-digit', minute: '2-digit' })}
                            </div>
                          </div>
                          {/* Green dot = submitted */}
                      ...
                      {liveResponses.length === 0 && (
                        <div style={{ textAlign: 'center', color: '#94a3b8', fontSize: 14, padding: '28px 0' }}>
                          <div style={{ fontSize: 32, marginBottom: 8 }}>⏳</div>
                          รอนักศึกษาตอบ...
                          <div style={{ fontSize: 12, marginTop: 6, color: '#cbd5e1' }}>อัปเดตอัตโนมัติทุก 5 วินาที</div>
                        </div>
                      )}
                    </div>
                  </div>
              ...
              return (
                <div key={s.id} style={{
                  ...
                  {(s.responses || []).length > 0 && (
                    <>
                      <button onClick={() => setViewingSession(s)} style={{
                        padding: '5px 11px', borderRadius: '8px', border: `1px solid ${CI.cyan}`, background: `${CI.cyan}10`,
                        color: CI.cyan, cursor: 'pointer', fontSize: '13px', fontFamily: 'inherit', fontWeight: 600,
                      }}>📊 ดูผล</button>
                      <button onClick={() => buildExcelFile(s)} style={{
                        padding: '5px 11px', borderRadius: '8px', border: '1px solid #16a34a', background: '#f0fdf4',
                        color: '#16a34a', cursor: 'pointer', fontSize: '13px', fontFamily: 'inherit', fontWeight: 600,
                      }}>📥 Excel</button>
                    </>
                  )}
                  {!isClosed && (
      ...
      {/* Results Modal */}
      {viewingSession && (
        <ResultsModal session={viewingSession} onClose={() => setViewingSession(null)} />
      )}
    </div>
  );
}
